...
8. **失效自省**：读不到宿主机内存信息时**拒绝扩容并持续高危告警**（绝不静默跳过）；主循环连续 100 次异常则退出交给 kubelet 重启。可通过 `WATCHDOG_ALLOW_BLIND_SCALEUP=true` 显式允许"盲扩半步长"。该状态由 `MemoryWatchdogHostStatsUnreadable` 告警暴露——注意它只在内存压力进入扩容路径后才触发，目标容器空闲期间的挂载损坏是观测不到的。
...
10. **可观测性**：`:8090/metrics` 暴露 Prometheus 指标（`watchdog_scale_up_total`、`watchdog_blocked_total{reason}`、`watchdog_working_set_bytes` 等），[`examples/servicemonitor.yaml`](../examples/servicemonitor.yaml) 提供 headless Service + ServiceMonitor，prometheus-operator 栈可自动抓取入库；扩缩容触发/生效/失败/拦截均以 **K8s Event** 挂到 pod 上（`kubectl describe pod` 直接可见，同类事件 60s 去重）；`:8090/healthz` 由主循环心跳驱动（停跳 30s 返回 503），liveness 探针探它，主循环卡死时由 kubelet 自动重启。**告警由监控栈统一承载**（PrometheusRule `memory-watchdog-rules` 共 7 条：resize 失败、宿主机枯竭、宿主机视野丢失、spec 读取失败、到达 cap、sidecar 反复重启、自身内存贴线；经 Alertmanager → 告警网关送达 IM 告警群）——watchdog 只负责"做"（秒级闭环处置）与打点，不自己发通知，"报警器自己死了"的盲区由外部观测覆盖。代码中保留的飞书通知器为历史遗留，没有任何地方注入其凭证，处于永久静默状态。
...
- **同节点多看门狗竞态**：各 Pod 的看门狗独立做宿主机红线校验，极端情况下会同时申请扩容。由于 requests 同步抬升，**kubelet 的 allocatable 准入是权威且串行的第二道闸**——竞态的后果从"宿主机超卖"降级为"后到的 resize 被判 Deferred/Infeasible 并回滚"。宿主机红线（90%）继续防御"邻居 pod 实际用量超出其 requests"这类 allocatable 记账覆盖不到的场景。缓解补充：PATCH 前抖动复核；以及在你这一侧，给被监督的工作负载加按节点打散的软约束（`topologySpreadConstraints`，`maxSkew: 1` / `whenUnsatisfiable: ScheduleAnyway`），从源头降低多个尖刺型 pod 同节点竞争借用空间的概率。
- **RBAC 粒度**：Role 无法限定"只能改自己"，watchdog SA 名义上可 patch 命名空间内任意 pod。两层缓解：① SA token 通过 projected volume 只挂给 watchdog 容器（pod 级 `automountServiceAccountToken: false`）；② [`examples/admission-policy.yaml`](../examples/admission-policy.yaml) 是一份 **ValidatingAdmissionPolicy**，越权请求被 API server 直接 Deny。`failurePolicy: Fail`（fail-closed）是安全的：matchConditions 限定只影响该 SA，策略故障的最坏结果是 resize 被拒、触发 `MemoryWatchdogResizeFailed` 告警。七条校验及各自封堵的越权向量：

  | # | 校验 | 封堵的越权向量 |
  |---|---|---|
  | 1 | 目标 pod 必须带 `name=my-worker` 标签 | resize / annotate 命名空间内的邻居 pod |
  ...
  | 5 | resize 请求中必须恰好存在一个 my-worker 容器 | 畸形请求绕过后续容器级校验 |
  | 6 | resize 时仅 my-worker 容器的 resources 可变 | 缩小同 pod 其他容器（如 watchdog 自身）制造定向 OOM |
  | 7 | my-worker 的 cpu requests/limits 不可变 | 越权调整 CPU（watchdog 只被授权动 memory） |
...
- **仅发布 amd64 与 arm64**：`publish-image` workflow 构建这两个平台。其他架构（ppc64le、s390x）可从源码构建（镜像是纯 Python + 多架构基础镜像），但不在发布列表内，需要时在 workflow 的 `platforms` 里添加即可。
- **sidecar 失效 = 失去弹性上探能力**：主容器按 `heavyResources` 原样启动（初始 limits 即 baseline），watchdog 负责在突发时把 limit 借升至 `maxMemoryFactor × baseline`。若 sidecar 持续不可用（集群版本不满足、hostPath 挂载损坏、CrashLoop），目标容器的内存上限就**停留在 baseline**（与不启用 watchdog 相同，不会更低），但突发任务失去 OOM 抢救。对应告警已随监控栈落地：`MemoryWatchdogSidecarRestarting`（重启率）、`MemoryWatchdogResizeFailed`、`MemoryWatchdogHostMemoryExhausted`、`MemoryWatchdogSpecReadErrors` 等 7 条规则见 `deploy/monitoring/prometheus-rules.yaml`。
...
扩容上限**只有倍数一种模式**：`cap = maxMemoryFactor × baseline`（baseline = 部署时目标容器的内存 limit，容器按其原样启动；默认倍数 2.0）。同一个值自动适配不同规格的工作负载（4Gi 的容器封顶 8Gi、16Gi 的封顶 32Gi），因此只在规模上有差异的多个部署可以复用同一份配置。刻意**不提供绝对值上限**——运行期实际能借到多少由宿主机红线和 kubelet allocatable 准入约束。`maxMemoryFactor ≤ 1` 会被钳制为 1（cap = baseline，watchdog 无法动作，通过 `BLOCKED_MAX_LIMIT` 告警暴露）。
