# Celery OOM 原地垂直扩缩容看门狗 Sidecar (Watchdog)

为 `celery-worker-heavy`（heavy Celery worker）配备的内存看门狗 Sidecar：高频采样主容器的 cgroup v2 **工作集内存**，在内核 OOM-kill 之前，通过 Kubernetes **原地垂直扩缩容（In-Place Pod Resize，`/resize` 子资源）** 抬高内存上限，任务结束后自动缩回，全程不重启容器、不打断长周期任务。

## 前提条件

| 条件 | 要求 | 不满足时的行为 |
|---|---|---|
| Kubernetes 版本 | **≥ 1.33**（EKS ≥ 1.34），`/resize` 子资源与原生 sidecar（initContainer `restartPolicy: Always`）均 GA | watchdog 首次 PATCH 收到 404/405 时打出明确 CRITICAL 日志并退出，反复重启触发 `CeleryWatchdogSidecarRestarting` 告警 |
...
3. **扩容（工作集 ≥ 80% limit）**：按步长（默认 2Gi，超出上限时截断）PATCH `/resize` 子资源，**requests 与 limits 同步抬升** —— 借用的内存全程纳入调度器/kubelet 记账：新 pod 无法被调度进这块空间；节点 allocatable 不足时 kubelet 判 `Infeasible`，由失败状态机处置。失败方向是"celery 拿不到内存（可能 pod OOM）"，而**宿主机与邻居 pod 始终安全**。
...
- **RBAC 粒度**：Role 无法限定"只能改自己"，watchdog SA 名义上可 patch 命名空间内任意 pod。两层缓解：① SA token 通过 projected volume 只挂给 watchdog 容器（pod 级 `automountServiceAccountToken: false`）；② chart 随附 **ValidatingAdmissionPolicy**（`watchdog-vap.yaml`，开关 `watchdog.admissionGuard`，默认开），越权请求被 API server 直接 Deny。`failurePolicy: Fail`（fail-closed）是安全的：matchConditions 限定只影响该 SA，策略故障的最坏结果是 resize 被拒、触发 `CeleryWatchdogResizeFailed` 告警。七条校验及各自封堵的越权向量：
  ...
  | 5 | resize 请求中必须恰好存在一个 celery-worker-heavy 容器 | 畸形请求绕过后续容器级校验 |
  | 6 | resize 时仅 celery-worker-heavy 容器的 resources 可变 | 缩小同 pod 其他容器（如 watchdog 自身）制造定向 OOM |
  | 7 | celery-worker-heavy 的 cpu requests/limits 不可变 | 越权调整 CPU（watchdog 只被授权动 memory） |
- **Infeasible 概率**：requests 抬升会占用节点 allocatable，节点紧张时扩容请求会被 kubelet 拒绝（Infeasible）——这是设计内的失败方向（celery 可能 OOM，宿主机安全），watchdog 会自动回滚 spec、告警并熔断 10 分钟。频繁出现说明节点容量真的不足，应扩节点或分流。
...
- **watchdog 自身内存画像与 exec 纪律**：sidecar 常驻约 70Mi（Python + kubernetes 客户端导入足迹为主），**不随 celery 内存规模或时间增长**，100Mi limit 实测余量充足。但**严禁在 watchdog 容器内 exec 会 import kubernetes 包的诊断脚本**——第二个解释器会再吃 60~80Mi，瞬间挤爆整个容器触发 OOMKilled；需要容器内诊断时只用轻量 stdlib 脚本，或直接看日志/指标。自身内存告警已落地（`CeleryWatchdogSelfMemoryHigh`：working set / limit > 90% 持续 10 分钟）。
...
压测脚本随本目录提供（`trigger_oom_test.py`），先拷进 `celery-worker-heavy` 容器再运行（目标 24G、每 1s 增长 100M、保持 30s 后释放）：

```bash
kubectl cp trigger_oom_test.py \
  <namespace>/<pod-name>:/tmp/trigger_oom_test.py -c celery-worker-heavy
kubectl exec -it <pod-name> -c celery-worker-heavy -n <namespace> -- \
...
| 现象 | 含义 | 处置 |
|---|---|---|
| 告警 `CeleryWatchdogResizeFailed` | 节点容量不足，kubelet 拒绝（Infeasible）或超时未应用 | 扩容节点或横向分流；watchdog 已自动回滚 spec 并熔断 10 分钟 |
...
| 告警 `CeleryWatchdogHostMemoryExhausted` | 节点整体拥塞，无安全空间 | 扩容节点；这是设计内的保护行为 |
| 告警 `CeleryWatchdogScaleUpBlockedAtCap` | 内存压力持续但已达 cap（factor × baseline） | 若为常态，上调该租户 baseline 或 maxMemoryFactor |
| 告警 `CeleryWatchdogSidecarRestarting` | sidecar 反复重启（前提不满足 / 端口占用 / 主循环持续异常 / 自身 OOM） | 看容器日志 CRITICAL 行；期间 heavy worker 上限停留在 baseline，无 OOM 抢救 |
| 告警 `CeleryWatchdogSpecReadErrors` | API Server 不可达，扩容决策被阻塞 | 检查 API Server / 网络；此状态下 OOM 抢救无法执行 |
| 告警 `CeleryWatchdogSelfMemoryHigh` | watchdog 自身内存贴近 100Mi limit | 排查是否有人 exec 了重型诊断进程；包升级抬高基线则提 limit 至 128Mi |
